...
def create_loader(batch_size, data_dir, data):
    loader = {'CIFAR10':cifar_10_loader,'CIFAR100': cifar_loader, 'TINY': tiny_loader, 'otherwise': imageset_loader}
...
def cifar_10_loader(batch_size, data_dir, data):
    num_label = 10
    ...
    trainset = datasets.CIFAR10(root=os.path.join(data_dir, data), train=True,
                                 download=True, transform=transform_train)
    testset = datasets.CIFAR10(root=os.path.join(data_dir, data), train=False,
...
def tiny_loader(batch_size, data_dir, data):
    ...
    testset = datasets.ImageFolder(root=os.path.join(data_dir, data, 'val'), transform=transform_test)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, pin_memory=True)
    return train_loader, test_loader, num_label
