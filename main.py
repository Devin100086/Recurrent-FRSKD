...
def main():
    ...
    # distill
    parser.add_argument('--use_distill', default='False')
    ...
    print(f"Total number of participants in the student model{sum(p.numel() for p in model.parameters() if p.requires_grad)}")
    print(f"Total number of participants in the teacher model{sum(p.numel() for p in bifpn.parameters() if p.requires_grad)}")
    ...
    if args.use_distill == "True":
        ...
        for epoch in range(1, args.epoch+1):
            s = time()
            loss, train_acc1, train_acc5 = train(model, bifpn, optimizer, criterion, train_loader, args)
            scheduler.step()
            torch.save(model, 'save/resnet18_entire_model.pth')
            print('finish! save model successfully!')
            test_acc1, test_acc5 = test(model, test_loader)
            logger.info('Epoch: {0:>2d}|Train Loss: {1:2.4f}| Train Acc: {2:.4f}| Test Acc1: {3:.4f}| Test Acc5: {3:.4f}| Time: {4:4.2f}(s)'
                        .format(epoch, loss, train_acc1, test_acc1, test_acc5, time() - s))
    # SAVE MODEL
        torch.save(model, 'save/resnet18_entire_model.pth')
        torch.save(bifpn, 'save/bifpn_entire_model.pth')
        logger.info('finish! save model successfully!')
    else:
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        scheduler = lr_scheduler(optimizer, args.scheduler, args.schedule, args.lr_decay, args.epoch)
        for epoch in range(1, args.epoch+1):
            s = time()
            loss, train_acc1 = train_without_distill(model, optimizer, criterion, train_loader, args)
            scheduler.step()
            test_acc1,_ = test(model, test_loader)
