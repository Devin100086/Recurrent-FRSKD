...
class CIFAR_ResNet(nn.Module):
    ...
    def forward(self, x, preact=False):
        ...
        return []
        """
        out = self.conv1(x) # [B, C1, H, W]
        out = self.bn1(out) # [B, C1, H, W]

        out1 = self.layer1(out)  # [B, 64, H, W]
        out2 = self.layer2(out1) # [B, 128, H/2, W/2]
        out3 = self.layer3(out2) # [B, 256, H/4, W/4]
        out4 = self.layer4(out3) # [B, 512, H/8, W/8]

        out = F.adaptive_avg_pool2d(F.relu(out4), (1,1)) # [B, 512, H/32, W/32]
        out = out.view(out.size(0), -1) # [B, (H * W) / 2]
        out = self.fc(out)
        if not preact:
            return [F.relu(out1), F.relu(out2), F.relu(out3), F.relu(out4)], out
        return [out1, out2, out3, out4], out
